...
import { type FinancialCategory, financeiroApi } from "@/lib/financeiro-api";
...
export default function DashboardFinanceiroCategorias() {
	...
	const [selected, setSelected] = useState<FinancialCategory | null>(null);
	const [deleteOpen, setDeleteOpen] = useState(false);

	const load = useCallback(async () => {
		setLoading(true);
		try {
			const result = await financeiroApi.listCategorias({
				q: search || undefined,
				page,
				pageSize,
			});
	...
	function openEdit(item: FinancialCategory) {
		setSelected(item);
		setForm({
			name: item.name,
			tipo: item.tipo,
	...
	async function handleSave() {
		...
		try {
			const body = {
				name: form.name.trim(),
				tipo: form.tipo,
				cor: form.cor.trim() || "#22c55e",
			};
			if (formMode === "create") {
				await financeiroApi.createCategoria(body);
				toaster.create({ title: "Categoria criada", type: "success" });
			} else if (selected) {
				await financeiroApi.updateCategoria(selected.id, body);
	...
	async function handleDelete() {
		if (!selected) return;
		try {
			await financeiroApi.removeCategoria(selected.id);
	...
	return (
		<Stack gap={6}>
			<Stack gap={1}>
				...
				<Text color="fg.muted">Categorias de receita e despesa</Text>
			</Stack>
									...
									<Table.Cell>
										<HStack gap={2}>
											<Box
												...
												h="16px"
												rounded="sm"
												bg={item.cor || "gray.400"}
												borderWidth="1px"
												borderColor="border"
											/>
											<Text fontSize="sm">{item.cor}</Text>
										</HStack>
									</Table.Cell>
